...
export default function RadarPerspectivasMobile({ promediosPorRol, promedioGlobalPorRol }: Props) {
  ...
  return (
    <div style={{ padding: '24px 20px 32px', display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'center' }}>
        <svg viewBox={`${-padX} ${viewBoxY} ${size + padX * 2} ${viewBoxH}`} style={{ width: '100%', maxWidth: 480, height: 'auto' }}>
          ...
          {DIMENSIONES.map((dim, i) => {
            ...
            return (
              <line key={dim.id} x1={center} y1={center} x2={x} y2={y} stroke="#0A0A0A" strokeOpacity={0.3} strokeWidth={1} />
            )
          })}
            ...
            return (
              <g key={rol}>
                ...
                {vals.map((v, i) => {
                  ...
                  return (
                    <circle key={i} cx={px} cy={py} r={4} fill={ROL_NEON[rol]} stroke="#0A0A0A" strokeWidth={1.5} />
                  )
                })}
              </g>
            )
          ...
          {[0, 2, 4, 6, 8, 10].map(level => {
            ...
            return (
              <g key={`tick-${level}`}>
                <circle cx={center} cy={cy} r={11} fill="#0A0A0A" />
                <text x={center} y={cy} fontSize="11" fontWeight="700" fill="#fff" textAnchor="middle" dominantBaseline="central">
                  {level}
                </text>
              </g>
            )
          ...
          {DIMENSIONES.map((dim, i) => {
            ...
            return (
              <text key={dim.id} x={x} y={y} textAnchor={textAnchor} dominantBaseline={dominantBaseline} fontSize="15" fontWeight="800" fill="#0A0A0A">
                {dim.nombre}
              </text>
            )
          })}

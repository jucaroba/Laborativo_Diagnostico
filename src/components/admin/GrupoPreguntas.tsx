            ...
            <TableRow key={p.id} style={i === preguntas.length - 1 && !agregando ? { borderBottom: 'none' } : undefined}>
              ...
              <TableCell style={{ width: 64, textAlign: 'right' }}>
                {editandoId === p.id ? (
                  ...
                  <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end' }} className="row-actions">
                    <button onClick={() => { setEditandoId(p.id); setTextoEdit(p.texto) }} style={iconBtn}><Pencil size={15} strokeWidth={2.25} color="var(--ink)" /></button>
